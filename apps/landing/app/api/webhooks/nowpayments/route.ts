/**
 * [TRADETOOLSET_NOWPAYMENTS_IPN] POST /api/webhooks/nowpayments
 *
 ...
 *    until then we log the verified event so journalctl on the deploy host
 *    has the audit trail.
 *
...
export async function POST(request: Request) {
  ...
  console.log(
    `[TRADETOOLSET_NOWPAYMENTS_IPN] verified=true order_id=${orderId} status=${status} paid=${paid}`
  );
