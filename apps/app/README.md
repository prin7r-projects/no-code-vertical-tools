# `apps/app` — Toolshelf operator dashboard (stubbed)
...
- The Toolshelf palette, typography, and component policy from `/DESIGN.md`
...
The landing's NOWPayments checkout already produces a real, IPN-verified invoice — the dashboard's job is to turn the verified order into an installed tool. Until that ships, the install step is handled manually over email by `orders@toolshelf.cash`.
